...
{
	GLfloat p[3] = { main_camera.position.x, main_camera.position.y, main_camera.position.z };
	...
	GLfloat f[3] = { c[0] - p[0], c[1] - p[1], c[2] - p[2] };
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	double x_delta = xpos - starting_press_x;
	double y_delta = ypos - starting_press_y;
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109061534 HW1", NULL, NULL);
    if (window == NULL)
    {
